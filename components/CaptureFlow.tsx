...
// ── Caption textarea with live hashtag + @mention highlighting + autocomplete ─
...
function CaptionInput({
  ...
  // position of the `@` character that started the current mention query
  ...
  function detectMention(text: string, cursorPos: number) {
    ...
    if (match) {
      ...
      return match[1]; // the partial username after @
    }
  ...
  function handleKeyUp(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    // Re-check on cursor movement (arrow keys, etc.)
