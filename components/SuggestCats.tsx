...
export function SuggestCats({ entryId }: { entryId: string }) {
  ...
  async function link(s: CatSuggestion) {
    setStates((prev) => ({ ...prev, [s.catId]: "saving" }));
    try {
      const res = await fetch(`/api/cat-entries/${entryId}/link`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ catId: s.catId }),
      });
      if (!res.ok) {
        setStates((prev) => ({ ...prev, [s.catId]: "idle" }));
        return;
      }
      const { status } = (await res.json()) as { status: "APPROVED" | "PENDING" };
      if (status === "APPROVED") {
        setStates((prev) => ({ ...prev, [s.catId]: "filed" }));
        router.refresh(); // the entry now shows its cat chip
      } else {
        setStates((prev) => ({ ...prev, [s.catId]: "requested" }));
      }
    } catch {
      setStates((prev) => ({ ...prev, [s.catId]: "idle" }));
    }
    ...
    <section className="mx-3 rounded-xl border border-dashed border-border bg-surface/60 p-4">
      ...
      <div className="flex flex-col gap-3">
        {suggestions.map((s) => {
          ...
          const state = states[s.catId] ?? "idle";
          return (
            <div key={s.catId} className="flex items-center gap-3">
              {cover ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={`/api/photos/${cover}`} alt={s.name} className="h-12 w-12 shrink-0 rounded-full object-cover ring-1 ring-border" />
              ) : (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted">
                  {s.isOwn ? t("suggestYours") : t("suggestBy", { name: ownerName })}
                </p>
              </div>
              {state === "filed" ? (
